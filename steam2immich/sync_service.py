...
def _discover_candidates(config: Config) -> tuple[list[ScreenshotCandidate], SyncSummary]:
    """Discover screenshots, enrich them, and build the initial sync summary."""
...
def _run_dry_run(candidates: list[ScreenshotCandidate], config: Config) -> None:
    """Write the dry-run report without preparing files or contacting Immich."""
...
def _add_to_album(
    ...
    """Add one uploaded asset to its target album and record completion."""
...
def _add_tags(
    ...
    """Apply Steam-related Immich tags to one uploaded asset."""
...
def print_summary(summary: SyncSummary) -> None:
    """Print the human-readable sync summary shown at the end of a run."""
...
def _filter_candidates(
    ...
    """Apply optional app ID and limit filters to discovered candidates."""
